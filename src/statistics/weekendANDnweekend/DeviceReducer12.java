 ...
 * 周末：非周末，宿舍区，人次
 */
public class DeviceReducer12 extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text _key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (Text val : values) {
			...
			if(Location.location.containsKey(strings[strings.length-1])&&!map.containsKey(strings[0]+strings[1])){
				map.put(strings[0]+strings[1], strings[0]+strings[1]);
			}
		}
		context.write(new Text("weekend"), new Text("nweekend  D,rc"));
